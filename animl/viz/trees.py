...
def dtreeviz(tree_model: (tree.DecisionTreeRegressor, tree.DecisionTreeClassifier),
    ...
    def instance_html(path, label_fontsize: int = 11):
        ...
        for i, name in enumerate(display_feature_names):
            ...
            headers.append(
                f'<td cellpadding="1" align="right" bgcolor="white"><font face="Helvetica" color="{color}" point-size="{label_fontsize}"><b>{name}</b></font></td>')
        ...
        for i, v in enumerate(display_X):
            ...
            values.append(
                f'<td cellpadding="1" align="right" bgcolor="white"><font face="Helvetica" color="{color}" point-size="{label_fontsize}">{disp_v}</font></td>')
    ...
    for node in shadow_tree.internal:
        ...
        if node.right.id in highlight_path:
            ...
            rpw = "1.2"
        edges.append(
            f'{nname}{fromport} -> {left_node_name}{toport} [penwidth={lpw} color="{lcolor}" label=<{llabel}>]')
        edges.append(
            f'{nname}{fromport} -> {right_node_name}{toport} [penwidth={rpw} color="{rcolor}" label=<{rlabel}>]')
        edges.append(f"""
        {{
            rank=same;
...
def class_split_viz(node: ShadowDecTreeNode,
    ...
    hist, bins, barcontainers = ax.hist(X_hist,
                                        ...
                                        align='mid',
                                        histtype=histtype,
                                        bins=np.arange(overall_feature_range[0], overall_feature_range[1] + binwidth,
                                                       binwidth),
                                        label=class_names)
